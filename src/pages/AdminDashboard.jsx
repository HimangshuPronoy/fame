    ...
    <div style={{ background: '#f0f2f5', minHeight: '100vh', display: 'flex' }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                gap: '1rem',
                padding: '1rem',
                background: activeTab === tab.id ? 'rgba(29, 191, 115, 0.15)' : 'transparent',
      ...
      <main style={{ flex: 1, padding: '3.5rem' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3rem' }}>
          <div>
            <h1 style={{ fontSize: '2rem', fontWeight: '900', color: '#111' }}>Owner Console</h1>
            <p style={{ color: '#62646a', marginTop: '0.25rem' }}>Manage and verify all establishments listed on Fame.</p>
          </div>
          <button 
          ...
          {[
            ...
            <div key={i} style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: '800', color: '#95979d', letterSpacing: '1px' }}>{s.label}</span>
                <span style={{ fontSize: '0.75rem', fontWeight: '700', color: s.color }}>{s.change}</span>
              </div>
              <div style={{ fontSize: '2.25rem', fontWeight: '800', color: '#111' }}>{s.val}</div>
            </div>
          ))}
        </div>

        {/* Content Tabs & Management Table */}
        <div style={{ background: 'white', border: '1px solid #e4e5e7', overflow: 'hidden' }}>
          <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #e4e5e7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fcfcfe' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: '700' }}>Inventory Records</h3>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" placeholder="Search records..." style={{ width: '240px', padding: '0.5rem 1rem', fontSize: '0.85rem' }} />
            </div>
          </div>
                ...
                {listings?.map(item => (
                  ...
                  <td style={{ padding: '1.5rem 2rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                      <img src={item.image} alt="" style={{ width: '64px', height: '40px', objectFit: 'cover', borderRadius: '2px' }} />
                      <div>
                        <div style={{ fontWeight: '700', fontSize: '1rem', color: '#111' }}>{item.name}</div>
      ...
      {showAddForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="animate-in" style={{ background: 'white', width: '100%', maxWidth: '600px', padding: '3.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2.5rem' }}>
              <h2 style={{ fontSize: '1.75rem', fontWeight: '800' }}>Register Establishment</h2>
              <button onClick={() => setShowAddForm(false)} style={{ background: 'transparent', color: '#95979d' }}><X size={28} /></button>
            </div>
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '800', fontSize: '0.8rem', color: '#95979d' }}>BUSINESS NAME</label>
                <input 
                  type="text" 
                  value={formData.name}
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '800', fontSize: '0.8rem', color: '#95979d' }}>CATEGORY</label>
                <input 
                  type="text" 
              ...
              <div style={{ marginBottom: '2.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '800', fontSize: '0.8rem', color: '#95979d' }}>DESCRIPTION</label>
                <textarea 
                  rows="4"
              ...
              <button type="submit" className="btn-primary" style={{ width: '100%', padding: '1.25rem', fontWeight: '800', fontSize: '1.05rem' }}>
                Add to Marketplace
              </button>
            </form>
          </div>
